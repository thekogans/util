...
namespace thekogans {
    namespace util {
        ...
        public:
            /// \brief
            /// ctor.
            /// NOTE: If you want the Console singleton to be created with custom
            /// ctor arguments, call Console::CreateInstance (...) with your custom
            /// arguments before the first call to Console::Instance ().
            /// \param[in] threadSafePrintString true == Serialize access to
            /// std::cout and std::cerr.
            /// \param[in] hookCtrlBreak true == Hook CTRL-C to call
            /// MainRunLoop::Instance ()->Stop ().
            /// \param[in] hookChild Linux and OS X only. true == Hook SIGCHLD
            /// to avoid zombie children.
            /// NOTE: You should only pass in true for hookChild if you're calling
            /// \see{ChildProcess::Spawn} (instead of \see{ChildProcess::Exec}, and
            /// you don't want to reap the zombie children yourself.
